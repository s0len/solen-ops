...
class Image implements IImage {
	...
	// Identifier of ICC profile chunks in JPEG APP2 segments
	protected const JPEG_ICC_IDENTIFIER = "ICC_PROFILE\x00";

	// Maximum ICC payload per JPEG APP2 segment: 65535 (max segment length)
	// - 2 (length field) - 12 (identifier) - 2 (chunk sequence number and count)
	protected const JPEG_ICC_MAX_CHUNK_SIZE = 65519;

	// ICC profiles are stored in metadata segments preceding the image data,
	// no need to scan further than this into the file
	protected const ICC_SCAN_BYTE_LIMIT = 8 * 1024 * 1024;
	...
	// ICC colour profile of the source image, carried over to the output
	// because GD strips it on load while keeping the (then mistagged) pixel values
	protected ?string $iccProfile = null;
	...
	/**
	 * Encodes the image to memory, re-embeds the ICC profile and writes the
	 * result to $filePath, or echoes it when no path is given (mirroring the
	 * behaviour of imagejpeg()/imagepng() with a null path).
	 */
	...
	/**
	 * Extracts the ICC colour profile from JPEG or PNG data and remembers it
	 * for later re-embedding. Only RGB profiles are kept: GD converts CMYK and
	 * grayscale images to RGB pixel data, so their source profiles no longer apply.
	 */
	protected function rememberIccProfile(string $data): void {
